...
impl Uuid {
    ...
    pub fn from_string(s: String) -> Result<Uuid, ParseError> {
        let len = s.len();
        if len != 36 {
            return Err(ParseError::InvalidLength(len));
        }
        ...
        b.append(&mut hex::decode(&s[0..8]).unwrap());
        b.append(&mut hex::decode(&s[9..13]).unwrap());
        b.append(&mut hex::decode(&s[14..18]).unwrap());
        b.append(&mut hex::decode(&s[19..23]).unwrap());
        b.append(&mut hex::decode(&s[24..36]).unwrap());

        copy_memory(&mut uuid.0, &b);
    ...
    fn get_time(&mut self) -> DateTime<Utc> {
        self.0[6] >> 4
    }
